...
const defaultReportPath = path.join(outputDir, 'local_regression_latest.html');
const defaultScreenshotPath = path.join(outputDir, 'local_regression_summary_mobile_layout_check.png');
...
function usage() {
  return [
    ...
    '  node tools/check_regression_summary_mobile_layout.mjs --json',
    '  node tools/check_regression_summary_mobile_layout.mjs --report <html> --screenshot <png>',
    '  node tools/check_regression_summary_mobile_layout.mjs --width 390 --height 1400',
    ...
    '',
    'This opens local_regression_latest.html in headless Chrome/Edge, captures a screenshot, and checks layout stability.',
...
function metricsExpression() {
  return `(() => { try {
    ...
    const visibleSteps = Array.from(document.querySelectorAll('.step')).filter(isVisible).length;
    const visiblePreBlocks = Array.from(document.querySelectorAll('pre')).filter(isVisible).length;
...
function buildChecks(metrics) {
  return [
    ...
    { name: 'crawler status text visible', ok: metrics.hasCrawlerStatusText === true },
    { name: 'n8n API credential hint visible', ok: metrics.hasCredentialHint === true },
    { name: 'file entry panel visible', ok: metrics.visibleEntryPanel === true },
    { name: 'artifact links visible', ok: metrics.visibleLinks >= 2 },
    { name: 'check steps visible', ok: metrics.visibleSteps >= 5 },
...
async function run(options) {
  ...
  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ai-selection-regression-summary-layout-'));
  let chrome;
...
function printHuman(result) {
  ...
  lines.push(`Local Regression Summary ${modeLabel} Layout Check`);
  ...
  lines.push(`- check steps: ${result.metrics.visibleSteps}`);
  lines.push(`- pre blocks: ${result.metrics.visiblePreBlocks}`);
  lines.push(`- credential hint visible: ${result.metrics.hasCredentialHint ? 'yes' : 'no'}`);
