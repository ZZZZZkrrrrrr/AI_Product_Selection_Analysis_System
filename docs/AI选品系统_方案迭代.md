...
## 实现迭代 80：统一回归总览摘要显示未知覆盖数量

### 本轮实现依据

- 迭代 75 已经让 `regression_overrides` 的未知 id 以提示方式保留，但总览首页只在配置提示区展示详情。
- V1.0 的运维入口应该先给出关键摘要，用户不需要展开或阅读长提示才能知道是否有未来回归项尚未生成。
- 本轮只增强统一回归总览的 JSON 和 HTML 摘要，不触发 n8n workflow，不改变商品分析主链路。

### 本轮改动

- `tools/build_local_regression_overview.mjs`
  - `counts` 新增 `unknown_overrides`。
  - 统一回归总览摘要卡新增“未知覆盖”指标。
  - 指标值来自 `regression_overrides.unknown_ids.length`，正常配置下为 `0`。
- `README_AI选品分析系统.md`
  - 补充说明未知覆盖 id 会进入总览摘要指标。

### 本轮验证

- 已完成代码和文档更新，下一步运行本地语法检查、配置校验、总览生成和一键回归。

### 本轮剩余风险

- 该指标只统计配置中未匹配到当前回归结果的 id，不判断未来是否一定会生成对应回归文件。
- 如果用户误拼 id，系统会提示但不阻断；需要根据配置提示区修正。
...
## 实现迭代 84：统一回归总览增加 GitHub 同步状态

### 本轮实现依据

- 用户已经要求把项目更新到 GitHub，后续自动迭代必须避免“本地已改但远端未同步”的状态不透明。
- 迭代 83 已有“运维结论”，但还没有把 GitHub 同步纳入判断。
- 本轮只读取本地 Git 状态，不访问外部业务 API，不触发 n8n workflow，不改变商品分析主链路。

### 本轮改动

- `tools/build_local_regression_overview.mjs`
  - 新增 `git_status_summary`。
  - 读取当前分支、upstream、origin URL、本地提交、远端提交、待推送数量、待拉取数量、已跟踪未提交数量、未跟踪数量。
  - 远端 URL 做基础脱敏，避免意外展示 HTTPS 凭据。
  - HTML 新增“GitHub 同步”区块。
  - `operational_readiness` 纳入 GitHub 同步判断：本地领先、落后、分叉或有已跟踪未提交改动时进入“需关注”。
  - 未跟踪项只展示数量和示例，不阻断同步判断，避免误伤用户本地临时目录。
- `tools/check_regression_overview_layout.mjs`
  - 增加 GitHub 同步区块、同步事实和同步文案可见性检查。
- `README_AI选品分析系统.md`
  - 补充统一回归总览会展示 GitHub 同步状态。

### 本轮验证

- 已完成代码和文档更新，下一步运行语法检查、刷新总览和一键回归。

### 本轮剩余风险

- GitHub 同步状态依赖本地 Git upstream；如果用户换远端或改分支，需要先确保 upstream 正确。
- 未跟踪目录不会阻断运维结论；如果用户希望纳入版本管理，需要手动确认后再提交。

### 下一轮实现建议

1. 尝试用 n8n API 做一次低频、受控的 workflow 只读/触发能力验证，确认授权不仅能读，还能支持自动执行。
2. 把运维结论同步到 `local_regression_latest.html` 的第一屏。
3. 增加一个只读命令，专门输出“当前是否可发布/可演示”的短报告。

## 实现迭代 85：本地回归报告第一屏同步运维结论

### 本轮实现依据

- 迭代 83 已经让统一回归总览具备“运维结论”，但用户日常首先打开的也可能是 `local_regression_latest.html`。
- V1.0 的非技术运维入口应该在单次回归报告第一屏也回答“现在能不能继续用”。
- 本轮只增强本地回归 HTML 展示和布局检查，不触发 n8n workflow，不改变商品分析主链路。

### 本轮改动

- `tools/run_local_regression_checks.mjs`
  - `buildSummary()` 增加本地 `operational_readiness`。
  - 在刷新统一回归总览后读取 `local_regression_overview.json`，同步更完整的 `operational_readiness`、`git_status_summary` 和 `data_source_health_summary`。
  - `local_regression_latest.html` 第一屏新增“运维结论”面板，显示“可继续使用 / 需关注”、依据、问题和 GitHub 同步摘要。
- `tools/check_regression_summary_mobile_layout.mjs`
  - 增加本地回归报告“运维结论”面板、依据标签和结论文案的布局检查。
- `README_AI选品分析系统.md`
  - 补充说明本地回归报告第一屏会显示运维结论，并在总览刷新后同步更完整状态。
...
- `local_regression_latest.json` 已包含：
  ...
  - `git_status_summary`
- `local_regression_latest.html` 已显示：
  - `运维结论`
  - `GitHub 同步`
  - `可继续使用 / 需关注` 状态文案
...
- 本地回归报告的运维结论在第一次写入时只基于本轮检查和 n8n 状态；最终写入会在统一回归总览刷新后补齐 GitHub 和数据源信息。
- 如果用户使用 `--no-overview`，本地回归报告会保留本轮检查级别的结论，不会同步统一总览结论。
